...
public class PayPalController {
    ...
    public String buy(Model model, @PathVariable(name = "orderId") Long orderId, Principal principal) {
        try {
            com.example.springboothw.entities.Order order = orderService.findById(orderId);

            Payer payer = new Payer();
            payer.setPaymentMethod("paypal");
            RedirectUrls redirectUrls = new RedirectUrls();
            redirectUrls.setCancelUrl("http://localhost:8189/app/paypal/cancel");
            redirectUrls.setReturnUrl("http://localhost:8189/app/paypal/success/" + order.getId());

            Amount amount = new Amount();
            amount.setCurrency("RUB");
            amount.setTotal(order.getCost().toString());

            Transaction transaction = new Transaction();
            transaction.setAmount(amount);
            transaction.setDescription("Покупка в Интернет магазине");
